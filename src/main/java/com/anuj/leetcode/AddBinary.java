...
public class AddBinary {
    ...
    public static String addBinary(String a, String b) {
        StringBuffer binary1Buffer = new StringBuffer(a);
        StringBuffer binary2Buffer = new StringBuffer(b);
        ...
        for (int i = 0; i < binary1Buffer.length() && i < binary2Buffer
                .length(); i++) {
            //the number additions can either be 0, 1, 10 or 11.
            //so the possible integers are 0, 1, 2, 3
            int sum = carryOver + (binary1Buffer.charAt(i) - '0') +
                    (binary2Buffer.charAt(i) - '0');
            carryOver = sum / 2;
            sum = sum % 2;
            answer.append(sum);
            lastIndexAdded = i;
        ...
        lastIndexAdded++;
        //Once the loop is done if we return.. we should be able to get the
        // right answer for all equal
        //length strings.
        StringBuffer rest = binary1Buffer.length() > binary2Buffer.length() ?
                binary1Buffer : binary2Buffer;
